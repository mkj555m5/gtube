...
fun MediaInfoDialog(
    ...
    var mediaInfo by remember { mutableStateOf<io.github.mahmoudmohsen.gtube.innertube.models.MediaInfo?>(null) }
    ...
    LaunchedEffect(track, video) {
        ...
        if (videoId != null) {
            mediaInfo = io.github.mahmoudmohsen.gtube.data.newmusic.InnertubeMusicService.getMediaInfo(videoId)
            resolvedDurationSeconds = mediaInfo?.durationSeconds?.takeIf { it > 0 }
                ?: resolvedDurationSeconds
                ?: io.github.mahmoudmohsen.gtube.data.music.YouTubeMusicService.fetchVideoDuration(videoId).takeIf { it > 0 }
        }
        isLoading = false
    }
